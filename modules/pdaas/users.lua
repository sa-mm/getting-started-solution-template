...
function u.listDevices(user, app_id, searchid, limit, offset)
  local query = {}
  ...
  if searchid and searchid ~= "" then
    query.identity = "^" .. searchid .. "." 
  end
...
function u.countDevices(user, app_id, searchid)
  local query = {}
  if searchid and searchid ~= "" then
    query.identity = "^" .. searchid .. "." 
  end
  query.tag = {name = "users", value = user.email}
  -- should implement a other tag filter with app_id,
  -- but not yet, related with ticket MUR-10779
  local res, error = Device2.countIdentities(query)
  if error then
    return nil, error
  end
  return res
