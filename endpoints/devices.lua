...
local function listDevices(user)
  ...
  local searchid = request.parameters.searchid
  local limit = request.parameters.limit
  local offset = request.parameters.offset
  ...
  return u.listDevices(user, app_id, searchid, limit, offset)
end
r:start(request):run(u.verifyToken):run(listDevices):response(response)

--#ENDPOINT GET /api/countdevices
local u = require("pdaas.users")
local r = require("pdaas.lib.router")
local e = require("pdaas.lib.errors")

local function countDevices(user)
  local app_id = request.parameters.app
  local searchid = request.parameters.searchid
  if app_id and app_id ~= "" then
    local hasapp = u.hasApp(user, app_id)
  ...
  return u.countDevices(user, app_id, searchid)
end
r:start(request):run(u.verifyToken):run(countDevices):response(response)
